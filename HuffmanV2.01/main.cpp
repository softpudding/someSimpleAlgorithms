...
int main() {
    ...
    int count=0;double lasttime=0;
    vector<unsigned int> vecOut;
    while(count<vec.size())
    {
        if((double)count/sum>lasttime+0.01)
        {
            cout<<(double)count/sum*100<<"%finished...\n";
            lasttime=(double)count/sum;
        }
        ch=vec[count];
        count+=1;
        ...
        else
        {
            ...
            vecOut.push_back(bit_1);
            bit_1=bit_2;
            bit_2=0;
        }
    }
    for(int i=0;i<vecOut.size();i++)
        fout.write((char*)&vecOut[i],sizeof(unsigned int));
