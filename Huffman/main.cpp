...
int main() {
    ...
    while(fin.get(ch))
    {
        ...
        if(pos+length(code)<=8)
        {
            bit_1+=(code-(1<<length(code)))<<(8-pos-length(code));
            pos=pos+length(code);
        }
        else
        {
            int leng_1=8-pos;//leng_1是这个bit里还可以存储的位数
            bit_1+=(code-(1<<length(code)))>>(length(code)-leng_1);
            bit_2+=(code-(1<<length(code)))<<(8-(length(code)-leng_1));
            pos=length(code)-leng_1;
            fout.write((char*)&bit_1,sizeof(char));
            bit_1=bit_2;
            bit_2=0;
        }
    }
